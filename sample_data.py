...
# generates sample financial time series data based on a Monte Carlo simulation for a gemoetric Brownian motion
#
import numpy as np
import pandas as pd
...
def generate_sample_data(rows, cols, freq='1min'):
    ...
    Parameters
    ----------
    rows: ints      number of rows to generate
    cols: int       number of columns to generate
    freq: str       frequency string for Datetimeindex

    Returns
    ----------
    df: DataFrame       DataFrame object with the sample data
